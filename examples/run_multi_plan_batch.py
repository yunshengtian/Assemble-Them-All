...
def seq_plan(asset_folder, assembly_dir, seq_planner_name, path_planner_name,
    rotation, body_type, sdf_dx, collision_th, force_mag, frame_skip,
    seq_max_time, path_max_time, seed, record_dir, save_dir, n_save_state):

    render = record_dir is not None

    clear_saved_sdfs(assembly_dir)
    seq_planner = get_seq_planner(seq_planner_name)(asset_folder, assembly_dir)
...
if __name__ == "__main__":
    ...
    for assembly_id in assembly_ids:
        assembly_dir = os.path.join(assemblies_dir, assembly_id)
        worker_args.append([
            asset_folder, assembly_dir, args.seq_planner, args.path_planner,
            args.rotation, args.body_type, args.sdf_dx, args.collision_th, args.force_mag, args.frame_skip,
            args.seq_max_time, args.path_max_time, args.seed, args.record_dir, args.save_dir, args.n_save_state
        ])
    ...
    try:
        for id, status, sequence, seq_count, t_plan in parallel_execute(seq_plan, worker_args, args.num_proc, show_progress=True):
            if status not in result_status:
                result_status[status] = {}
            result_status[status][id] = [seq_count, t_plan, sequence]
