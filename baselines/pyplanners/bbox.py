...
def transform_vectors(vectors, config, com=np.zeros(3)):
    if len(config) == 3: # translation only
        ...
        return (vectors - com).dot(rotation.T) + translation + com
    else:
...
def get_bbox_all(vertices, configs, com):
    min_box, max_box = np.zeros((0, 3)), np.zeros((0, 3))
    for config in configs:
        transformed_vertices = transform_pts_by_state(vertices, state=config, com=com)
        min_box = np.vstack([min_box, transformed_vertices.min(axis=0)])
        max_box = np.vstack([max_box, transformed_vertices.max(axis=0)])
    return min_box, max_box


def get_bbox_goal_test(vertices_move, vertices_still, com_move=np.zeros(3)):
    ...
    def goal_test(config):
        if len(config) == 3: # translation only
            ...
            vertices_move_new = transform_vectors(vertices_move, config, com=com_move)
...
def compute_nearest_bbox_goal(nodes, vertices_move, vertices_still, com_move=np.zeros(3)):
    ...
    if nodes_configs.shape[1] == 3: # translation only
        ...
        min_box_move, max_box_move = get_bbox_all(vertices_move, nodes_configs, com_move)
        min_box_still, max_box_still = get_bbox(vertices_still)
        size_box_move = max_box_move - min_box_move
