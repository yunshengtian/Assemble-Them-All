...
// private
// // original damping model (proportional to ddot) and friction related to damping force as well
// void ForceGeneralPrimitiveContact::computeForce(std::vector<Contact> &contacts, VectorX& fm) {
...
//         Vector6 ni1 = Gamma1.transpose() * (R1.transpose() * n);
//         Vector6 ni2 = Gamma2.transpose() * (R2.transpose() * n);

//         // contact force
//         Vector6 fc1 = (-_kn * d - _damping * ddot) * ni1;
//         fm.head(6) += fc1;
//         Vector6 fc2 = (_kn * d + _damping * ddot) * ni2;
//         fm.tail(6) += fc2;

//         // std::cerr << "fc2 = " << fc2.transpose() << std::endl;
//         // std::cerr << "contact force magnitude = " << _kn * d + _damping * ddot << std::endl;
...
//             if (_mu * fc_norm >= _kt * tdot_norm - constants::eps) {  // static frictional force
//                 Vector6 fs = -_kt * Gamma1.transpose() * (R1.transpose() * tdot);
//                 fm.head(6) += fs;
//             } else {                                // dynamic frictional force
//                 Vector6 fd = -_mu * fc_norm / tdot_norm * Gamma1.transpose() * (R1.transpose() * tdot);
//                 fm.head(6) += fd;
//             }
//             // primitive contact body
//             if (_mu * fc_norm >= _kt * tdot_norm - constants::eps) {  // static frictional force
//                 Vector6 fs = _kt * Gamma2.transpose() * (R2.transpose() * tdot);
//                 fm.tail(6) += fs;
//             } else {                                // dynanmic frictional force
//                 Vector6 fd = _mu * fc_norm / tdot_norm * Gamma2.transpose() * (R2.transpose() * tdot);
//                 fm.tail(6) += fd;
//             }
//         }
//     }
...
//         Matrix6 dfc1_dq1 = - ni1 * (_kn * dd_dq1 + _damping * dddot_dq1) -
//                             (_kn * d + _damping * ddot) * GTRT1 * dn_dq1;
//         dfc1_dq1.leftCols(3) -= (_kn * d + _damping * ddot) * Gamma1.transpose() * math::skew(R1.transpose() * n);
//         Km.topLeftCorner(6, 6) += dfc1_dq1;

//         Matrix6 dfc1_dq2 = - ni1 * (_kn * dd_dq2 + _damping * dddot_dq2) - 
//                             (_kn * d + _damping * ddot) * GTRT1 * dn_dq2;
//         Km.topRightCorner(6, 6) += dfc1_dq2;

//         Matrix6 dfc1_dphi1 = - ni1 * _damping * dddot_dphi1;
//         Dm.topLeftCorner(6, 6) += dfc1_dphi1;

//         Matrix6 dfc1_dphi2 = -ni1 * _damping * dddot_dphi2;
...
//         Km.bottomLeftCorner(6, 6) += ni2 * (_kn * dd_dq1 + _damping * dddot_dq1) + 
//                                         (_kn * d + _damping * ddot) * (tmp1 + GTRT2 * dn_dq1);
//         Matrix6 tmp2;
//         tmp2.setZero();
//         for (int j = 0;j < 6;j++)
//             tmp2.col(j) = dGamma2_dq2(j).transpose() * (R2.transpose() * n);
//         Km.bottomRightCorner(6, 6) += ni2 * (_kn * dd_dq2 + _damping * dddot_dq2) + 
//                                         (_kn * d + _damping * ddot) * (tmp2 + GTRT2 * dn_dq2);
//         Km.block(6, 6, 6, 3) +=
//             (_kn * d + _damping * ddot) * Gamma2.transpose() * math::skew(R2.transpose() * n);

//         Dm.bottomLeftCorner(6, 6) += ni2 * _damping * dddot_dphi1;
//         Dm.bottomRightCorner(6, 6) += ni2 * _damping * dddot_dphi2;
...
//                 // derivatives
//                 Km.topLeftCorner(6, 6) -= _kt * GTRT1 * dtdot_dq1;
//                 Km.block(0, 0, 6, 3) -= _kt * Gamma1.transpose() * math::skew(R1.transpose() * tdot);
//                 Km.topRightCorner(6, 6) -= _kt * GTRT1 * dtdot_dq2;

//                 Dm.topLeftCorner(6, 6) -= _kt * GTRT1 * dtdot_dphi1;
//                 Dm.topRightCorner(6, 6) -= _kt * GTRT1 * dtdot_dphi2;
//             } else {                                // dynamic frictional force
...
//                 Km.bottomLeftCorner(6, 6) += _kt * GTRT2 * dtdot_dq1 + _kt * tmp1;
                
//                 Matrix6 tmp2;
//                 tmp2.setZero();
//                 for (int j = 0;j < 6;j++)
//                     tmp2.col(j) = dGamma2_dq2(j).transpose() * (R2.transpose() * tdot);
//                 Km.bottomRightCorner(6, 6) += _kt * GTRT2 * dtdot_dq2 + _kt * tmp2;
//                 Km.block(6, 6, 6, 3) += _kt * Gamma2.transpose() * math::skew(R2.transpose() * tdot);
                
//                 Dm.bottomLeftCorner(6, 6) += _kt * GTRT2 * dtdot_dphi1;
//                 Dm.bottomRightCorner(6, 6) += _kt * GTRT2 * dtdot_dphi2;
//             } else {                                // dynanmic frictional force
//                 Vector6 fd = _mu * fc_norm / tdot_norm * Gamma2.transpose() * (R2.transpose() * tdot);
//                 fm.tail(6) += fd;
...
// // original damping model (proportional to ddot) and friction does not related to damping force
// void ForceGeneralPrimitiveContact::computeForce(std::vector<Contact> &contacts, VectorX& fm) {
//     Matrix3 R1 = _contact_body->_E_0i.topLeftCorner(3, 3);
//     Vector6 phi1 = _contact_body->_phi;
//     Matrix3 R2 = _primitive_body->_E_0i.topLeftCorner(3, 3);
...
//         if (_mu > constants::eps) {
//             // general contact body
//             if (-_mu * _kn * d >= _kt * tdot_norm - constants::eps) {  // static frictional force
//                 Vector6 fs = -_kt * Gamma1.transpose() * (R1.transpose() * tdot);
//                 fm.head(6) += fs;
//             } else {                                // dynamic frictional force
//                 Vector6 fd = _mu * _kn * d / tdot_norm * Gamma1.transpose() * (R1.transpose() * tdot);
//                 fm.head(6) += fd;
//             }
//             // primitive contact body
//             if (-_mu * _kn * d >= _kt * tdot_norm - constants::eps) {  // static frictional force
...
//                 fm.tail(6) += fs;
//             } else {                                // dynanmic frictional force
//                 Vector6 fd = - _mu * _kn * d / tdot_norm * Gamma2.transpose() * (R2.transpose() * tdot);
//                 fm.tail(6) += fd;
//             }
//         }
//     }
...
//                 // derivatives
//                 Km.topLeftCorner(6, 6) +=
//                     _mu / tdot_norm * _kn * GTRT1 * (tdot * dd_dq1 - d / tdot_norm * tdot * dtdot_norm_dq1 + d * dtdot_dq1);
//                 Km.block(0, 0, 6, 3) +=
//                     _mu * _kn * Gamma1.transpose() * math::skew(d / tdot_norm * R1.transpose() * tdot);
//                 Km.topRightCorner(6, 6) +=
//                     _mu / tdot_norm * _kn * GTRT1 * (tdot * dd_dq2 - d / tdot_norm * tdot * dtdot_norm_dq2 + d * dtdot_dq2);

//                 Dm.topLeftCorner(6, 6) +=
//                     _mu / tdot_norm * _kn * GTRT1 * (- d / tdot_norm * tdot * dtdot_norm_dphi1 + d * dtdot_dphi1);
//                 Dm.topRightCorner(6, 6) +=
//                     _mu / tdot_norm * _kn * GTRT1 * (- d / tdot_norm * tdot * dtdot_norm_dphi2 + d * dtdot_dphi2);
//             }
...
//                     tmp1.col(j) = dGamma2_dq1(j).transpose() * (R2.transpose() * tdot);
//                 Km.bottomLeftCorner(6, 6) -= 
//                     _mu / tdot_norm * _kn * GTRT2 * (tdot * dd_dq1 - tdot * d / tdot_norm * dtdot_norm_dq1 + d * dtdot_dq1) +
//                     _mu * _kn * d / tdot_norm * tmp1;
...
//                     tmp2.col(j) = dGamma2_dq2(j).transpose() * (R2.transpose() * tdot);
//                 Km.bottomRightCorner(6, 6) -= 
//                     _mu / tdot_norm * _kn * GTRT2 * (tdot * dd_dq2 - tdot * d / tdot_norm * dtdot_norm_dq2 + d * dtdot_dq2) +
//                     _mu * _kn * d / tdot_norm * tmp2;
//                 Km.block(6, 6, 6, 3) -= 
//                     _mu * _kn * Gamma2.transpose() * math::skew(d / tdot_norm * R2.transpose() * tdot);

//                 Dm.bottomLeftCorner(6, 6) -= 
//                     _mu / tdot_norm * _kn * GTRT2 * (- tdot * d / tdot_norm * dtdot_norm_dphi1 + d * dtdot_dphi1);
//                 Dm.bottomRightCorner(6, 6) -= 
//                     _mu / tdot_norm * _kn * GTRT2 * (- tdot * d / tdot_norm * dtdot_norm_dphi2 + d * dtdot_dphi2);
//             }
//         }
//     }
// }

// // new damping model (proportional to ddot * d) and friction does not related to damping force
// void ForceGeneralPrimitiveContact::computeForce(std::vector<Contact> &contacts, VectorX& fm) {
//     Matrix3 R1 = _contact_body->_E_0i.topLeftCorner(3, 3);
//     Vector6 phi1 = _contact_body->_phi;
//     Matrix3 R2 = _primitive_body->_E_0i.topLeftCorner(3, 3);
...
//         // std::cerr << "xw = " << xw1.transpose() << ", xw1_dot = " << xw1_dot.transpose() << ", n = " << n.transpose() << std::endl;
//         // std::cerr << "normal = " << n.transpose() << std::endl;
//         // std::cerr << "normal.norm = " << n.norm() << std::endl;
//         // std::cerr << "d = " << d << std::endl;
//         // std::cerr << "ddot = " << ddot << std::endl;
...
//         // contact force
//         Vector6 fc1 = (-_kn * d + _damping * ddot * d) * ni1;
//         fm.head(6) += fc1;
//         Vector6 fc2 = (_kn * d - _damping * ddot * d) * ni2;
//         fm.tail(6) += fc2;
...
//         if (_mu > constants::eps) {
//             // general contact body
//             // std::cerr << "static: " << _kt * tdot_norm << ", dynamic = " << -_mu * _kn * d << std::endl << ", tdot = " << tdot.transpose() << std::endl;
//             if (-_mu * _kn * d >= _kt * tdot_norm - constants::eps) {  // static frictional force
//                 Vector6 fs = -_kt * Gamma1.transpose() * (R1.transpose() * tdot);
//                 fm.head(6) += fs;
//             } else {                                // dynamic frictional force
...
//         Matrix6 dfc1_dq1 = - ni1 * (_kn * dd_dq1 - _damping * dddot_dq1 * d - _damping * ddot * dd_dq1) -
//                             (_kn * d - _damping * ddot * d) * GTRT1 * dn_dq1;
//         dfc1_dq1.leftCols(3) -= (_kn * d - _damping * ddot * d) * Gamma1.transpose() * math::skew(R1.transpose() * n);
//         Km.topLeftCorner(6, 6) += dfc1_dq1;

//         Matrix6 dfc1_dq2 = - ni1 * (_kn * dd_dq2 - _damping * dddot_dq2 * d - _damping * ddot * dd_dq2) - 
//                             (_kn * d - _damping * ddot * d) * GTRT1 * dn_dq2;
//         Km.topRightCorner(6, 6) += dfc1_dq2;
...
//         for (int j = 0;j < 6;j++)
//             tmp1.col(j) = dGamma2_dq1(j).transpose() * (R2.transpose() * n);
//         Km.bottomLeftCorner(6, 6) += ni2 * (_kn * dd_dq1 - _damping * dddot_dq1 * d - _damping * ddot * dd_dq1) + 
//                                         (_kn * d - _damping * ddot * d) * (tmp1 + GTRT2 * dn_dq1);
//         Matrix6 tmp2;
//         tmp2.setZero();
//         for (int j = 0;j < 6;j++)
//             tmp2.col(j) = dGamma2_dq2(j).transpose() * (R2.transpose() * n);
//         Km.bottomRightCorner(6, 6) += ni2 * (_kn * dd_dq2 - _damping * dddot_dq2 * d - _damping * ddot * dd_dq2) + 
//                                         (_kn * d - _damping * ddot * d) * (tmp2 + GTRT2 * dn_dq2);
//         Km.block(6, 6, 6, 3) +=
//             (_kn * d - _damping * ddot * d) * Gamma2.transpose() * math::skew(R2.transpose() * n);
...
//             // frictional force on the general contact body
//             if (verbose) {
//                 std::cerr << -_mu * _kn * d << ", " << _kt * tdot_norm << std::endl;
//                 std::cerr << "d = " << d << ", " << ",tdot = " << tdot.transpose() << ", tdot_norm = " << tdot_norm << std::endl;
//             }
//             if (-_mu * _kn * d >= _kt * tdot_norm - constants::eps) {  // static frictional force
//                 // std::cerr << "static" << std::endl;
//                 Vector6 fs = -_kt * Gamma1.transpose() * (R1.transpose() * tdot);
